...
MainWindow::MainWindow(const QString &windowTitle, QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    move(QGuiApplication::primaryScreen()->geometry().center() - this->geometry().center()); // position window on screen center
    setWindowIcon(theIcon); // main icon displayed on top left of main window
    setAttribute(Qt::WA_DeleteOnClose, true); // make sure windows get deleted when they are closed
}
